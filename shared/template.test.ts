...
describe('validateTemplateDoc', () => {
  ...
  it('rejeita formato desconhecido, versão futura e frente ausente', () => {
    ...
    expect(validateTemplateDoc({ format: 'impress-o/template', version: 0.5, name: 'x', orientation: 'landscape', front: { fabric: { objects: [] } } }).ok).toBe(false)
    expect(validateTemplateDoc({ format: 'impress-o/template', version: 1, name: 'x', orientation: 'landscape', front: { fabric: { objects: 'nope' } } }).ok).toBe(false)
  })
  it('só marca frente e verso quando o verso existe', () => {
    ...
    const r1 = validateTemplateDoc({ ...base, doubleSided: true, back: {} })
    expect(r1.ok && r1.doc.doubleSided).toBe(false)
