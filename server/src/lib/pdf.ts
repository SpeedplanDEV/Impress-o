 ...
 */
export async function buildCardPdf(opts: { frontPng: Buffer; backPng?: Buffer | null; orientation: 'landscape' | 'portrait'; rotate180?: boolean }): Promise<Buffer> {
  ...
  const pageW = landscape ? wPt : hPt
  const pageH = landscape ? hPt : wPt
