...
| `server/src/index.ts` + `lib/startup.ts` | Inicialização: versão mínima do Node (22.13), validação de `PORT`, escuta em 127.0.0.1 e ::1 (ou no `HOST` informado), detecta um Impress-o já aberto antes de tocar no banco (sai com código 42, que o `iniciar.bat` reconhece); porta ocupada sem `PORT` fixa → tenta 3071–3074 e 8070–8074; dicas em português para falhas (banco na nuvem, pasta sem permissão, SQLite bloqueado), nunca dicas de banco para erros de porta, abertura do navegador (`IMPRESSO_OPEN_BROWSER=1`) |
